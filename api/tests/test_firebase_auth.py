"""
Script d'utilitat per testejar l'autenticació Firebase JWT

Aquest script demostra com obtenir un token JWT de Firebase i fer crides a l'API.
Per utilitzar-lo, necessites tenir un usuari creat a Firebase Authentication.
"""
import requests
import json
import sys

def test_authenticated_endpoint(base_url, uid, token):
    """
    Testa un endpoint autenticat
    """
    print("\n" + "="*60)
    print("TEST: Endpoint autenticat - GET /api/users/{uid}/")
    print("="*60)
    ...
    try:
        ...
        if response.status_code == 200:
            print("\n✅ Test PASSAT - Usuari obtingut correctament")
            return True
